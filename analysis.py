...
import requests
from config import MPSTATS_API_URL, HEADERS, logger, ADMIN_USERNAMES, CATCOM_PATH
...
async def analyze_command(update, context, admin_ids, admin_usernames):
    ...
    # Инициализируем калькулятор комиссий
    try:
        commission_calc = CommissionCalculator(CATCOM_PATH)
        print("[analysis] Калькулятор комиссий инициализирован")
    except Exception as e:
        print(f"[analysis] Ошибка инициализации калькулятора: {e}")
        ...
        try:
            ...
            if results:
                for r in results:
                    ...
                    # ДОБАВЛЯЕМ КОМИССИЮ С ПЕРЕДАЧЕЙ КАТЕГОРИИ ПОЛЬЗОВАТЕЛЯ
                    if commission_calc:
                        try:
                            product_name = r.get('name', '')
                            price = r.get('price', 0)
                            # Передаем категорию для умного поиска
                            commission = commission_calc.get_commission(
                                product_name, 
                                price, 
                                user_category=name  # категория из шаблона пользователя
                            )
                            r['Комиссия'] = commission
                        except Exception as e:
                            print(f'[analysis] Ошибка расчета комиссии: {e}')
                            r['Комиссия'] = None
                    else:
                        r['Комиссия'] = None
